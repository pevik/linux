...
/* Maximum lock time can be 150 * PDIV cycles */
#define PLL141XX_LOCK_FACTOR    (150)
...
#define PLL141XX_LOCK_STAT_SHIFT (29)
#define PLL141XX_ENABLE_SHIFT    (31)

static unsigned long samsung_pll141xx_recalc_rate(struct clk_hw *hw,
				unsigned long parent_rate)
{
	struct samsung_clk_pll *pll = to_clk_pll(hw);
	u32 mdiv, pdiv, sdiv, pll_con;
	...
	pll_con = readl_relaxed(pll->con_reg);
	mdiv = (pll_con >> PLL141XX_MDIV_SHIFT) & PLL141XX_MDIV_MASK;
	pdiv = (pll_con >> PLL141XX_PDIV_SHIFT) & PLL141XX_PDIV_MASK;
	sdiv = (pll_con >> PLL141XX_SDIV_SHIFT) & PLL141XX_SDIV_MASK;
...
static inline bool samsung_pll141xx_mp_change(
		const struct samsung_pll_rate_table *rate, u32 pll_con)
{
	u32 old_mdiv, old_pdiv;

	old_mdiv = (pll_con >> PLL141XX_MDIV_SHIFT) & PLL141XX_MDIV_MASK;
	old_pdiv = (pll_con >> PLL141XX_PDIV_SHIFT) & PLL141XX_PDIV_MASK;

	return (rate->mdiv != old_mdiv || rate->pdiv != old_pdiv);
}

static int samsung_pll141xx_set_rate(struct clk_hw *hw, unsigned long drate,
					unsigned long prate)
{
	struct samsung_clk_pll *pll = to_clk_pll(hw);
	const struct samsung_pll_rate_table *rate;
	...
	if (!(samsung_pll141xx_mp_change(rate, tmp))) {
		/* If only s change, change just s value only*/
		tmp &= ~(PLL141XX_SDIV_MASK << PLL141XX_SDIV_SHIFT);
		tmp |= rate->sdiv << PLL141XX_SDIV_SHIFT;
		writel_relaxed(tmp, pll->con_reg);
	...
	/* Set PLL lock time. */
	writel_relaxed(rate->pdiv * PLL141XX_LOCK_FACTOR,
			pll->lock_reg);

	/* Change PLL PMS values */
	tmp &= ~((PLL141XX_MDIV_MASK << PLL141XX_MDIV_SHIFT) |
			(PLL141XX_PDIV_MASK << PLL141XX_PDIV_SHIFT) |
			(PLL141XX_SDIV_MASK << PLL141XX_SDIV_SHIFT));
	tmp |= (rate->mdiv << PLL141XX_MDIV_SHIFT) |
			(rate->pdiv << PLL141XX_PDIV_SHIFT) |
			(rate->sdiv << PLL141XX_SDIV_SHIFT);
	tmp &= ~(1 << 26);
	tmp |= (1 << 5);
	writel_relaxed(tmp, pll->con_reg);
...
static const struct clk_ops samsung_pll141xx_clk_ops = {
	.recalc_rate = samsung_pll141xx_recalc_rate,
	.round_rate = samsung_pll_round_rate,
	.set_rate = samsung_pll141xx_set_rate,
	.enable = samsung_pll_enable,
	.disable = samsung_pll_disable,
};

static const struct clk_ops samsung_pll141xx_clk_min_ops = {
	.recalc_rate = samsung_pll141xx_recalc_rate,
};
...
/* Maximum lock time can be 150 * PDIV cycles */
#define PLL1419X_LOCK_FACTOR    (150)

#define PLL1419X_MDIV_MASK       (0x3FF)
#define PLL1419X_PDIV_MASK       (0x3F)
#define PLL1419X_SDIV_MASK       (0x7)
#define PLL1419X_MDIV_SHIFT      (16)
#define PLL1419X_PDIV_SHIFT      (8)
#define PLL1419X_SDIV_SHIFT      (0)
#define PLL1419X_LOCK_STAT_SHIFT (29)
#define PLL1419X_ENABLE_SHIFT    (31)

static unsigned long samsung_pll1419x_recalc_rate(struct clk_hw *hw,
				unsigned long parent_rate)
{
	struct samsung_clk_pll *pll = to_clk_pll(hw);
	u32 mdiv, pdiv, sdiv, pll_con;
	...
	pll_con = readl_relaxed(pll->con_reg);
	mdiv = (pll_con >> PLL1419X_MDIV_SHIFT) & PLL1419X_MDIV_MASK;
	pdiv = (pll_con >> PLL1419X_PDIV_SHIFT) & PLL1419X_PDIV_MASK;
	sdiv = (pll_con >> PLL1419X_SDIV_SHIFT) & PLL1419X_SDIV_MASK;
...
static inline bool samsung_pll1419x_mp_change(
		const struct samsung_pll_rate_table *rate, u32 pll_con)
{
	u32 old_mdiv, old_pdiv;

	old_mdiv = (pll_con >> PLL1419X_MDIV_SHIFT) & PLL1419X_MDIV_MASK;
	old_pdiv = (pll_con >> PLL1419X_PDIV_SHIFT) & PLL1419X_PDIV_MASK;

	return (rate->mdiv != old_mdiv || rate->pdiv != old_pdiv);
}

static int samsung_pll1419x_set_rate(struct clk_hw *hw, unsigned long drate,
					unsigned long prate)
{
	struct samsung_clk_pll *pll = to_clk_pll(hw);
	const struct samsung_pll_rate_table *rate;
	...
	if (!(samsung_pll141xx_mp_change(rate, tmp))) {
		/* If only s change, change just s value only*/
		tmp &= ~(PLL1419X_SDIV_MASK << PLL1419X_SDIV_SHIFT);
		tmp |= rate->sdiv << PLL1419X_SDIV_SHIFT;
		writel_relaxed(tmp, pll->con_reg);
	...
	/* Set PLL lock time. */
	writel_relaxed(rate->pdiv * PLL1419X_LOCK_FACTOR,
			pll->lock_reg);

	/* Change PLL PMS values */
	tmp &= ~((PLL1419X_MDIV_MASK << PLL1419X_MDIV_SHIFT) |
			(PLL1419X_PDIV_MASK << PLL1419X_PDIV_SHIFT) |
			(PLL1419X_SDIV_MASK << PLL1419X_SDIV_SHIFT));
	tmp |= (rate->mdiv << PLL1419X_MDIV_SHIFT) |
			(rate->pdiv << PLL1419X_PDIV_SHIFT) |
			(rate->sdiv << PLL1419X_SDIV_SHIFT);
	writel_relaxed(tmp, pll->con_reg);
...
static const struct clk_ops samsung_pll1419x_clk_ops = {
	.recalc_rate = samsung_pll1419x_recalc_rate,
	.round_rate = samsung_pll_round_rate,
	.set_rate = samsung_pll1419x_set_rate,
	.enable = samsung_pll_enable,
	.disable = samsung_pll_disable,
};

static const struct clk_ops samsung_pll1419x_clk_min_ops = {
	.recalc_rate = samsung_pll1419x_recalc_rate,
};
				...
				const struct samsung_pll_clock *pll_clk)
{
	...
	case pll_2650xx:
		...
		break;
	case pll_141xx:
		pll->enable_offs = PLL141XX_ENABLE_SHIFT;
		pll->lock_offs = PLL141XX_LOCK_STAT_SHIFT;
		if (!pll->rate_table)
			init.ops = &samsung_pll141xx_clk_min_ops;
		else
			init.ops = &samsung_pll141xx_clk_ops;
		break;
	case pll_1419x:
		pll->enable_offs = PLL1419X_ENABLE_SHIFT;
		pll->lock_offs = PLL1419X_LOCK_STAT_SHIFT;
		if (!pll->rate_table)
			init.ops = &samsung_pll1419x_clk_min_ops;
		else
			init.ops = &samsung_pll1419x_clk_ops;
		break;
	default:
